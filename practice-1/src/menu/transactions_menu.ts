...
export async function transactionMenu() {
  ...
  switch (option) {
    case "Compra": {
      ...
      inventory.buyAssets(result.selected_merchant, result.date, ...result.purchases);
      console.table(inventory.getStockReport().map((stock) => ({
        name: stock[0].name,
        quantity: stock[1]
      })));

      console.table(inventory.transactions.filter((trans) => trans instanceof BuyTransaction).map((trans) => ({
        date: trans.date.getDate(),
        assets: trans.getExchangeAssets().map((stock) => "[" + stock[0].name + ", " + stock[1].toString() + "]").join(", "),
        merchant: (trans as BuyTransaction).merchant.name,
        crowns: trans.crowns
      })));
      break;
    }
    ...
    case "Venta": {
      ...
      inventory.sellAssets(result_sell.selected_client, result_sell.date_Sell, ...result_sell.purchases_sell);
      console.table(inventory.getStockReport().map((stock) => ({
        name: stock[0].name,
        quantity: stock[1]
      })));

      console.table(inventory.transactions.filter((trans) => trans instanceof SellTransaction).map((trans) => ({
        date: trans.date.getDate(),
        assets: trans.getExchangeAssets().map((stock) => "[" + stock[0].name + ", " + stock[1].toString() + "]").join(", "),
        client: (trans as SellTransaction).client.name,
        crowns: trans.crowns
      })));
      break;
    }
    ...
    case "Devolución de un objeto": {
      ...
      inventory.refundSellAssets(result_buy.selected_client, result_buy.date_Sell, ...result_buy.purchases_sell);
      console.table(inventory.getStockReport().map((stock) => ({
        name: stock[0].name,
        quantity: stock[1]
      })));

      console.table(inventory.transactions.filter((trans) => trans instanceof RefundSellTransaction).map((trans) => ({
        date: trans.date.getDate(),
        assets: trans.getExchangeAssets().map((stock) => "[" + stock[0].name + ", " + stock[1].toString() + "]").join(", "),
        client: (trans as RefundSellTransaction).client.name,
        crowns: trans.crowns
      })));
      break;
    }
    ...
    case "Reembolso de dinero": {
      ...
      inventory.refundBuyAssets(result_refund.selected_merchant, result_refund.date, ...result_refund.purchases);
      console.table(inventory.getStockReport().map((stock) => ({
        name: stock[0].name,
        quantity: stock[1]
      })));

      console.table(inventory.transactions.filter((trans) => trans instanceof RefundBuyTransaction).map((trans) => ({
        date: trans.date.getDate(),
        assets: trans.getExchangeAssets().map((stock) => "[" + stock[0].name + ", " + stock[1].toString() + "]").join(", "),
        merchant: (trans as RefundBuyTransaction).merchant.name,
        crowns: trans.crowns
      })));
      break;
    }
...
/**
 * Solicita al usuario los datos para realizar una transacción de compra y reembolso
...
async function buyAndRefund(merchant_name: string): Promise<{ selected_merchant: Merchant; date: Date; purchases: Stock[] } | undefined> {
  ...
  while (continuar) {
    ...
    const { buy_more } = await inquirer.prompt([
      { type: "confirm", name: "buy_more", message: "¿Desea realizar otra acción?", default: false }
    ]);
    continuar = buy_more;
  }
...
/**
 * Solicita al usuario los datos para realizar una transacción de venta y devolución
 * @param client_name - El nombre del cliente al que se le realizará la venta
 */
async function sellAndRefund(client_name: string): Promise<{ selected_client: Clients; date_Sell: Date; purchases_sell: Stock[] } | undefined> {
  ...
  while (continuar_sell) {
    ...
    const { sell_more } = await inquirer.prompt([
      { type: "confirm", name: "sell_more", message: "¿Desea realizar otra acción?", default: false }
    ]);
