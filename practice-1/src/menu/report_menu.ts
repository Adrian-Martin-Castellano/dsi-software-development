 ...
 * Ingresos y Gastos: Te muestra un balance entre el dinero ingresado y el dinero gastado.
 * Historial de transacciones: Te muestra el historial de transacciones de un cliente o de un mercader.
...
export async function reportMenu() {
  ...
  switch (option) {
    case "Estado del Stock": {
      ...
      const report_stock = inventory.getStockReport(search_name);
    ...
    case "Mejores Ventas": {
      const stock_demand = inventory.getBestSellingAssets();
    ...
    case "Ingresos y Gastos": {
      const financy_result = inventory.getFinancialSummary();
      console.table({
    ...
    case "Historial de transacciones": {
      ...
      switch (option_history) {
        case "Historial de un cliente": {
          ...
          const client_history = inventory.getTransactionHistoryForClient(client);
        ...
        case "Historial de un mercader": {
          ...
          const mercader = MyMerchants.merchants.find(m => m.name.toLowerCase() === mercader_name.toLowerCase());
          if (!mercader) {
            console.log("No se encontró un mercader con ese nombre.");
            break;
          }
          const mercader_history = inventory.getTransactionHistoryForMerchant(mercader);
          if (typeof mercader_history === "undefined") {
            console.log("No existe historial para este mercader.");
            break;
          }
