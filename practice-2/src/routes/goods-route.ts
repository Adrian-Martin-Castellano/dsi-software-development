...
/**
 * POST /goods handler. Allows storing a good document in the database.
 */
...
/**
 * GET /goods handler. Retrieves good documents matching fields provided via query string parameters.
 */
...
/**
 * GET /goods/:id handler. Retrieves a good document by its unique ID passed as a path parameter.
 */
...
/**
 * PATCH /goods handler. Updates a good document filtered by query string parameters.
 */
goodRouter.patch('/goods', async (req, res) => {
  ...
  if (!req.query.name && !req.query.description && !req.query.weight && !req.query.crowns) {
    res.status(400).send({
      error: 'A field filter must be provided in the query string',
    });
  } else if (!req.body) {
    res.status(400).send({
...
/**
 * PATCH /goods/:id handler. Updates a good document using its ID passed as a path parameter.
 */
...
/**
 * DELETE /goods handler. Removes a good document matching fields provided via query string.
 */
goodRouter.delete('/goods', async (req, res) => {
  ...
  if (!req.query.name && !req.query.description && !req.query.weight && !req.query.crowns) {
    res.status(400).send({
      error: 'At least one field parameter must be provided',
    });
  } else {
    try {
...
/**
 * DELETE /goods/:id handler. Removes a good document by its ID passed as a path parameter.
 */
